...
--     it "comprimentos não devem mudar" $
--       (length $ match "edil" "e") `shouldBe` (length "edil")
--     context "ao receber listas vazias" $ do
--       it "match \"edil\" \"\" should be \"____\"" $
--         match "edil" "" `shouldBe` "____" 
...
-- main :: IO ()
-- main = do
--   quickCheck (withMaxSuccess 1000 prop_reverse)
...
main = hspec $ do
  describe "quicksort" $ do
    ...
    modifyMaxSuccess (const 1000) $ it "length :: [Int]" $
      property (prop_length :: [Int] -> Bool)
    it "minimum :: [Int]" $
      property (prop_minimum :: [Int] -> Property)
